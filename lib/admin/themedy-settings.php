...
function themedy_register_theme_settings() {
	register_setting(  LINEITUP_SETTINGS_FIELD,  LINEITUP_SETTINGS_FIELD );
	add_option(  LINEITUP_SETTINGS_FIELD, themedy_theme_settings_defaults() );

	if ( !isset($_REQUEST['page']) || $_REQUEST['page'] != 'themedy' )
		return;

	if ( themedy_get_option('reset') ) {
		update_option( LINEITUP_SETTINGS_FIELD, themedy_theme_settings_defaults());
...
?>	
	<div class="wrap">
        ...
        <h2>
            <?php _e(CHILD_THEME_NAME.' - Theme Settings', 'themedy'); ?>
        </h2>
        ...
        <form method="post" action="options.php">
        	<p class="top-buttons">
                ...
                <input type="submit" class="button" name="<?php echo  LINEITUP_SETTINGS_FIELD; ?>[reset]" value="<?php _e('Reset Settings', 'themedy'); ?>" onclick="return confirm('<?php echo esc_js( __('Are you sure you want to reset?', 'themedy') ); ?>');" />
            </p>
    
            <?php wp_nonce_field('closedpostboxes', 'closedpostboxesnonce', false ); ?>
            <?php wp_nonce_field('meta-box-order', 'meta-box-order-nonce', false ); ?>
            <?php settings_fields( LINEITUP_SETTINGS_FIELD); // important! ?>
            <input type="hidden" name="<?php echo  LINEITUP_SETTINGS_FIELD; ?>[theme_version]>" value="<?php echo esc_attr(themedy_option('theme_version')); ?>" />
